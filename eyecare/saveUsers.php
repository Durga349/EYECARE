...
if (isset($_POST['action']) && $_POST['action'] == 'show'){
   // $general_hel_show = "SELECT * FROM ".$tablename."  ORDER BY id DESC";
      ...
      $users_show = "SELECT * FROM `personal_information`  order by id desc";
   ...
   $date = $_POST['date'];
    ...
    if ($date) {
     ...
     $users_show = "SELECT  * FROM `personal_information` ".$condition." order by id desc";
      ...
      // $count_query = "SELECT COUNT(*) AS datecount FROM `personal_information` " . $condition;
      // $count_data = $crud->getData($count_query);
      // $datecount = $count_data[0]['datecount'];
 ...
 if (isset($_POST['action']) && $_POST['action'] == 'showMonthData') {
   $month = $_POST['month'];

    if ($month) {
        $condition =" where LEFT(date, 7) = '".$_POST['month']."'";
    }else{
        $condition ="";
    }
